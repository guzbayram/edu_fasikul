// ═══════════════════════════════════════════════════════════
// EduFasikül — Faz 1: Vite ES Module bundle
// Bu dosya monolitik index.html'den taşındı.
// Davranış aynı kalacak şekilde sadece import'lar npm'e çevrildi.
// ═══════════════════════════════════════════════════════════

import './styles.css';

// ─── Firebase (eskiden CDN: gstatic.com/firebasejs/12.14.0) ───
...
// ─── Chart.js (eskiden CDN: chart.js/4.4.0) ───
import Chart from 'chart.js/auto';
window.Chart = Chart;

// ═══════════════════════════════════════════════════════════
// FIREBASE INIT (orijinal index.html satır 2342-2394)
// ═══════════════════════════════════════════════════════════
...
const appState = {
  ...
  timerCountdown: true,
  sorularState: {},  // {soruNo: {selected, correct, timeSec}}
  activeQuestionIdx: 0,
  readingMode: false,
  ...
  avatarEmojis: ['🎓','🧑‍🎓','👨‍🎓','👩‍🎓','🦁','🐯','🦊','🦅','🚀','⚡','🔥','🎯','💡','🌟','🏆','🎮','🤖','👾','🧠','🌈'],
  avatarIdx: 0,
  eduDirHandle: null,   // Edu-Fasikul lokal klasör erişimi
...
function _sanitizeEmailKey(email){ return email.toLowerCase().trim().replace(/[^a-zA-Z0-9]/g,'_'); }

// ─── GİRİŞ (Firebase Authentication) ───────────────────────────
async function doLogin(){
  ...
  try{
    ...
    // Kullanıcı profilini Firestore'dan çek (rol, ad, aktif mi?)
    const docRef = window._fsDoc(window._db, 'kullanicilar', uid);
    const snap = await window._fsGetDoc(docRef);

    if(!snap.exists()){
      // Admin hesabı yeniden oluşturulmuşsa (UID değişmiş) → profil otomatik oluştur
      ...
      // E-posta ile eski UID altındaki belgeyi ara (UID değişmişse geçiş yap)
      try{
      ...
      // Firestore'da hiç yok → yetkisiz hesap
      await window._authSignOut(window._auth);
...
function enterApp(name){
  ...
  // Admin menüsünü sadece admin rolüne göster
  ...
  // Firestore'dan veri yükle (misafir değilse)
  if(appState.user && appState.user.email !== 'misafir@demo.com'){
    setTimeout(()=>{ if(typeof loadFromFirestore==='function') loadFromFirestore(); }, 500);
  }

  // v4: First-time onboarding
  if(window._showOnboardOnLogin){
    window._showOnboardOnLogin = false;
    setTimeout(()=>{
      showToast('İlk girişin! Sana kısa bir tur yapayım 👋','info');
      setTimeout(startOnboarding, 2500);
    }, 800);
  }
}
...
// ──────────────────────────────────────────────────────────────────
// ADMİN — KULLANICI YÖNETİMİ (Firebase Auth + Firestore)
// Kullanıcı ekleme: Auth'ta hesap oluşturur + Firestore'a profil yazar.
// Şifreler artık Firestore'da tutulmaz; Auth sunucusunda güvenli şekilde saklanır.
// ──────────────────────────────────────────────────────────────────
async function loadKullaniciList(){
...
async function addKullanici(){
  ...
  try{
    ...
    // 2) Firestore'a profil yaz (şifre YOK — Auth'ta güvenli)
    const docRef = window._fsDoc(window._db, 'kullanicilar', uid);
    ...
    // 3) Admin hâlâ giriş yapmış kalsın — yeni kullanıcı hesabını bırak
    //    (createUserWithEmailAndPassword mevcut oturumu DEĞİŞTİRMEZ,
    //    ancak bazı eski SDK versiyonlarında değiştirebilir. Güvenli tarafta kalalım.)
    // Firebase v12'de createUser mevcut oturumu etkilemez; bu yüzden ek bir şey gerekmez.
    ...
    if(code.includes('email-already-in-use')){
      // Auth'ta kayıtlı ama Firestore dokümanı silinmiş olabilir.
      // Verilen şifreyle giriş deneyerek UID'yi kurtaralım.
      try {
...
async function deleteKullanici(uid){
  ...
  try{
    ...
    // Not: Firebase Auth hesabını silmek için Cloud Function gerekir (client SDK ile
    // sadece kendi hesabını silebilirsin). Firestore profili silinince giriş yapsa bile
    // "Bu hesap yönetici tarafından eklenmemiş" hatasını alır → erişimi engellenir.
    showToast('Kullanıcı erişimi kaldırıldı','success');
    loadKullaniciList();
  }catch(e){
...
async function toggleKullaniciActive(uid, makeActive){
  try{
    const docRef = window._fsDoc(window._db,'kullanicilar', uid);
    await window._fsSetDoc(docRef, {active: makeActive}, {merge:true});
    showToast(makeActive ? 'Hesap etkinleştirildi' : 'Hesap devre dışı bırakıldı', 'success');
    loadKullaniciList();
  }catch(e){
...
async function resetKullaniciPassword(email){
  if(!email){ showToast('E-posta bulunamadı','error'); return; }
  ...
  try{
    ...
    showToast('E-posta gönderilemedi: ' + (e.message||e.code),'error');
  }
}
...
async function openReader(dersId, fasikulId){
  closeDrawer();
  if(isGuestSession() && !GUEST_DEMO_FASIKUL_IDS.has(fasikulId)){
  ...
  if(fasikul.sourceType==='bundled'){
    const source=BUNDLED_FASIKUL_SOURCES.find(s=>s.id===fasikul.id || s.json===fasikul.jsonFile);
    if(source){
...
function closeReader(){
  ...
  if(appState.aktifFasikul){
    ...
    const fasRef = dersId
      ? MANIFEST.dersler.find(d=>d.id===dersId)?.fasikuller.find(f=>f.id===fas.id)
      : null;
...
function getDashboardStats(){
  ...
  if(!useCalculated && pending.toplam){
    Object.entries(pending.konular||{}).forEach(([konu,k])=>{
      if(!konular[konu]) konular[konu]={dogru:0,yanlis:0};
      konular[konu].dogru=Number(konular[konu].dogru||0)+Number(k.dogru||0);
      konular[konu].yanlis=Number(konular[konu].yanlis||0)+Number(k.yanlis||0);
    });
  }
...
function _getUserKey(){
  ...
  if(!appState.user || appState.user.email==='misafir@demo.com') return null;
  // UID varsa onu kullan (Firebase Auth), yoksa e-postayı key'e çevir (eski veri geçiş desteği)
  return appState.user.uid || appState.user.email.replace(/[^a-zA-Z0-9]/g,'_');
}
function _safeDocId(value){ return encodeURIComponent(String(value||'')).replace(/\./g,'%2E'); }
function _userDocRef(uid=_getUserKey()){
...
function _hesaplaFasikulIstatistik(){
  const fasikuller = {};
  (appState.hatalilar||[]).forEach(h=>{
...
function persistDrawingCloud(key,json){
  const uid=_getUserKey();
...
function deleteDrawingCloud(key){
  const uid=_getUserKey();
...
// Firestore onSnapshot ile cozumler ve cizimler koleksiyonlarını dinler.
// Başka bir cihazdan gelen değişiklik <1sn içinde UI'a yansır.

function startRealtimeSync(uid){
  stopRealtimeSync(); // Önceki listener varsa temizle
  ...
  window._realtimeUnsubCozumler = window._fsOnSnapshot(cozumlerRef, (snapshot)=>{
    ...
    snapshot.docChanges().forEach(change=>{
      ...
      // Kendi yazdığımız değişiklikleri atla
      if(change.doc.metadata.hasPendingWrites) return;
      // Daha yeni veri geliyorsa güncelle
    ...
    if(changed){
      ...
      recalcFasikulProgress();
      updateDashboard();
      // Fasikül kartlarını güncelle
      if(typeof renderDerslerGrid==='function') renderDerslerGrid();
      // Reader açıksa test progressini ve soru şeridini güncelle
      const readerOpen = document.getElementById('reader-overlay')?.classList.contains('open');
      if(readerOpen){
        if(typeof updateTestProgress==='function') updateTestProgress();
        if(appState.aktifAltKonu?.sorular && typeof renderSoruStrip==='function'){
          renderSoruStrip(appState.aktifAltKonu.sorular);
        }
      }
    }
  ...
  window._realtimeUnsubCizimler = window._fsOnSnapshot(cizimlerRef, (snapshot)=>{
    snapshot.docChanges().forEach(change=>{
      if(change.type==='removed') return;
      // Kendi yazdığımız değişiklikleri atla (local write henüz sunucuya gitmemiş)
      if(change.doc.metadata.hasPendingWrites) return;
      const data = change.doc.data();
      const key = data.key;
      if(!key || !data.json) return;
      if(appState.drawings[key] === data.json) return; // Aynı içerik, gerek yok
      appState.drawings[key] = data.json;
      // Şu an bu sayfa açıksa canvas'ı yenile
      const aktifId = appState.aktifFasikul?.id;
      const currentPage = appState.currentPage;
      const currentKey = aktifId ? `drawing_${aktifId}_p${currentPage}` : null;
      if(currentKey === key){
        // Canvas başlatılmışsa anında yükle
        const fc = appState.fabricCanvases?.[currentPage] || appState.fabricCanvas;
        if(fc){
          fc.loadFromJSON(data.json, ()=>{ fc.renderAll(); });
        } else {
          // Canvas henüz yok — sayfa açıldığında otomatik yüklenir (drawings[] zaten güncellendi)
          // Kısa süre sonra tekrar dene (PDF yükleniyor olabilir)
          setTimeout(()=>{
...
async function loadFromFirestore(){
  ...
  try{
    ...
    if(snap.exists()){
      ...
      // Eski ana belge formatını da destekle; yeni formatta asıl veri alt koleksiyondadır.
      if(data.sorularState) appState.sorularState = data.sorularState;
      localStorage.setItem('edu_hatalilar',    JSON.stringify(appState.hatalilar));
    }

    // Eski koleksiyondan otomatik geçiş: ogrenciler/{uid} veya ogrenciler/{emailKey}
    ...
    showToast('Veriler buluttan yüklendi ☁️','success');
    // Gerçek zamanlı senkronizasyonu başlat
